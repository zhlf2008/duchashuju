    ...
    <div>
      ...
      <Modal title={editingId ? '编辑学期' : '新增学期'} open={modalVisible} onCancel={() => { setModalVisible(false); setEditingId(null); form.resetFields() }} onOk={() => form.submit()} width={500}>
        <Form form={form} onFinish={editingId ? handleEdit : handleAdd} layout="vertical">
          <Form.Item name="semester_name" label="学期名称" rules={[{ required: true, message: '请输入学期名称' }]} style={{ marginBottom: 12 }}>
            <Input placeholder="如：2026年春季学期" />
          </Form.Item>
          <Form.Item name="trial_range" label="试晨读日期范围" tooltip="试晨读在正式学期开始前，非必填" style={{ marginBottom: 12 }}>
            <RangePicker style={{ width: '100%' }} format="YYYY-MM-DD" placeholder={['试晨读开始', '试晨读结束']} />
          </Form.Item>
          <Form.Item name="date_range" label="正式学期日期范围" rules={[{ required: true, message: '请选择日期范围' }]} style={{ marginBottom: 12 }}>
            <RangePicker style={{ width: '100%' }} format="YYYY-MM-DD" placeholder={['开始日期', '结束日期']} />
          </Form.Item>
          <Form.Item name="is_current" label="设为当前学期" valuePropName="checked" style={{ marginBottom: 0 }}>
            <Switch />
          </Form.Item>
        </Form>
      </Modal>
